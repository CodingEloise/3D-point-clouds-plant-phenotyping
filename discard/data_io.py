...
def load_as_array(path):
	...
	data_array = np.loadtxt(data_dir + scans[i], comments='//') # raw data as np array, of shape (nx6) or (nx8) if labels are available.
...
if __name__ == '__main__':
	...
	for i in range(0, 84):
		pointcloud, labels_available, labels = load_as_o3d_cloud(data_dir + scans[i])
		filename = os.path.join("E:/3D-point-clouds-plant-phenotyping/output/", f"point_cloud{i}.ply")
